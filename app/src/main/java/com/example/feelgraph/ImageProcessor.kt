...
class ImageProcessor {
    ...
    fun processImage(bitmap: Bitmap): List<Line>{
        ...
        //Eliminacion de ruido mediante filtro gaussiano
    ...
    private fun convertBitmapToGrayscaleMatHandlingTransparency(bitmap: Bitmap): Mat {
        ...
        // Comprueba si el Bitmap tiene transparencia
        if (bitmap.hasAlpha()) {
            // Obtiene el canal alpha y verifica la transparencia
            val alphaChannel = ArrayList<Mat>(4)
            Core.split(matWithAlpha, alphaChannel)
            val alphaMat = alphaChannel[3]

            // Crea una máscara donde los píxeles transparentes se marcarán con blanco
            val mask = Mat(alphaMat.size(), alphaMat.type(), Scalar(255.0))
            Core.compare(alphaMat, Scalar(0.0), mask, Core.CMP_EQ)

            // Rellena los píxeles transparentes con blanco en el Mat original
    ...
    private fun excludeYAxis(lines: List<Line>): List<Line> {
        ...
        // Identifica la línea vertical más a la izquierda que es potencialmente el eje Y
        val yAxis = lines.filter { abs(it.p1.x - it.p2.x) < 10 }
            .minByOrNull { it.p1.x } // Elige la línea con la coordenada 'x' más pequeña, que sería la linea situada mas a la izquierda
    ...
    private fun excludeAxesLines(lines: List<Line>): List<Line> {
        ...
        val xAxis = lines.filter { abs(it.p1.y - it.p2.y) < 10 }
            .maxByOrNull { max(it.p1.y, it.p2.y) }
    ...
    private fun findIntersectionPoints(lines: List<Line>, xAxis: Line?, threshold: Double = 10.0): List<Point> {
        ...
        if(xAxis != null) {
            ...
            lines.forEach { line ->
                // Comprueba si el inicio o el fin del segmento está cerca del eje X
    ...
    private fun filterCloseIntersectionPoints(intersectionPoints: List<Point>, distanceThreshold: Double = 10.0): List<Point> {
        ...
        for (point in sortedPoints.drop(1)) {
            // Comprobación de si el punto está más allá del umbral de distancia desde el inicio del grupo actual
    ...
    private fun divideAndFilterXAxis(xAxis: Line?, intersectionPoints: List<Point>): List<Line> {
        ...
        // Añade segmentos intermedios, alternando entre mantener y eliminar
        ...
        if(xAxis != null) {
            if (sortedIntersectionPoints.size % 2 != 0) {
                // Añade el último tramo si la cantidad de puntos de intersección es impar
